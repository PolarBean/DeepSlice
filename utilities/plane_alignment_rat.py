...
def get_axis(m, translation_vector, direction, plane_of_section=None):
    '''
    ...
    print(f'axis_tvol: {translated_volume}')
    print(f'axis_tvec: {translation_vector}')
...
#     sag_axis = ((translated_volume[1] / 2, depth, translated_volume[2] / 2))
    ...
    if plane_of_section is None:
        plane_of_section = np.argmin(np.abs((cor_Y - 512, sag_X - 256, horz_Z - 256)))
    choices = {'x':  sag_X - 256, ' y': cor_Y-512,' z':  horz_Z - 256 }
...
#     axis = ((translated_volume[0] / 2, depth, translated_volume[1] / 2))

    
#     expr = sum((x * cross + midpoint) * cross) - k
#     m = solve(expr)
#     axis = np.array((m * cross + midpoint), dtype=np.float)
    ##this gives me the depth of a point directly beneath the coronal center point
...
def rotate_section(section, degrees, direction, plane_of_section=None):
    '''
    Rotates a section
...
def Section_adjust(section, direction, mean, plane='Coronal'):
    '''
    Adjusts a section
    :param section: the section to adjust
    :type section: :any:`numpy.ndarray`
    :param direction: the direction of the adjustment
    :type direction: string
    :param mean: the target angle to adjust to
    :type mean: float
    :param plane: whether the section is coronal, sagittal, horizontal
    :type plane: string
    :returns: the adjusted section
    :rtype: :any:`numpy.ndarray`
    '''
    cross, k = find_plane_equation(section)
    angle = get_angle(section, cross, k, direction)
    dif = angle-mean
    rot = (rotate_section(section, -dif, direction, plane))
    return(rot)
